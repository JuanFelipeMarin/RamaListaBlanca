	...
	BEGIN
		...
		END
			...
			SELECT		DISTINCT dbo.fFloatAVarchar(prospecto.cli_identifi) AS 'id',
						...
						CAST('Cortesía' AS VARCHAR(50)) AS 'documentType',
  ...
  set @bitAccesoPorReservaWeb = (select bitAccesoPorReservaWeb from gim_configuracion_ingreso where cdgimnasio = @gymId and intfkSucursal = @branchId)
	...
	SET @action = '11111'
		...
		BEGIN
			...
			FROM	WhiteList
			WHERE	id = @id
			AND		((WhiteList.invoiceId = 0 AND WhiteList.documentType = 'Cortesía' 
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF EXISTS(SELECT * FROM WhiteList WHERE gymId = @intIdGimnasio AND invoiceId = @intNumeroFactura AND documentType = 'Cortesía')
			BEGIN

			SET @ID = (SELECT DISTINCT id FROM WhiteList WHERE gymId = @intIdGimnasio AND invoiceId = @intNumeroFactura AND documentType = 'Cortesía')
			set @strTipoPersona =(SELECT DISTINCT UPPER(typePerson) FROM WhiteList WHERE gymId = @intIdGimnasio AND invoiceId = @intNumeroFactura AND documentType = 'Cortesía')
